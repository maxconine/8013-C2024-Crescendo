...
// public StatorCurrentLimitConfiguration STATOR_CURRENT_LIMIT = new
// StatorCurrentLimitConfiguration(true, 300, 700,
// 1);
// public SupplyCurrentLimitConfiguration SUPPLY_CURRENT_LIMIT = new
// SupplyCurrentLimitConfiguration(false, 40, 100,
// 1);
...
// TalonUtil.checkError(mMaster.configSelectedFeedbackSensor(TalonFXFeedbackDevice.IntegratedSensor,
// 0,
// Constants.kLongCANTimeoutMs), mConstants.kName + ": Could not detect encoder:
// ");

// mForwardSoftLimitTicks = (int) ((mConstants.kMaxUnitsLimit -
// mConstants.kHomePosition)
// * mConstants.kTicksPerUnitDistance);
// TalonUtil.checkError(
// mMaster.configForwardSoftLimitThreshold(mForwardSoftLimitTicks,
// Constants.kLongCANTimeoutMs),
// mConstants.kName + ": Could not set forward soft limit: ");

// TalonUtil.checkError(mMaster.configForwardSoftLimitEnable(true,
// Constants.kLongCANTimeoutMs),
// mConstants.kName + ": Could not enable forward soft limit: ");

// mReverseSoftLimitTicks = (int) ((mConstants.kMinUnitsLimit -
// mConstants.kHomePosition)
// * mConstants.kTicksPerUnitDistance);
// TalonUtil.checkError(
// mMaster.configReverseSoftLimitThreshold(mReverseSoftLimitTicks,
// Constants.kLongCANTimeoutMs),
// mConstants.kName + ": Could not set reverse soft limit: ");

// TalonUtil.checkError(mMaster.configReverseSoftLimitEnable(true,
// Constants.kLongCANTimeoutMs),
// mConstants.kName + ": Could not enable reverse soft limit: ");

// TalonUtil.checkError(mMaster.configVoltageCompSaturation(12.0,
// Constants.kLongCANTimeoutMs),
// mConstants.kName + ": Could not set voltage compensation saturation: ");

// TalonUtil.checkError(mMaster.config_kP(kMotionProfileSlot, mConstants.kKp,
// Constants.kLongCANTimeoutMs),
// mConstants.kName + ": could not set kP: ");

// TalonUtil.checkError(mMaster.config_kI(kMotionProfileSlot, mConstants.kKi,
// Constants.kLongCANTimeoutMs),
// mConstants.kName + ": could not set kI: ");

// TalonUtil.checkError(mMaster.config_kD(kMotionProfileSlot, mConstants.kKd,
// Constants.kLongCANTimeoutMs),
// mConstants.kName + ": could not set kD: ");

// TalonUtil.checkError(mMaster.config_kF(kMotionProfileSlot, mConstants.kKf,
// Constants.kLongCANTimeoutMs),
// mConstants.kName + ": Could not set kF: ");

// TalonUtil
// .checkError(mMaster.configMaxIntegralAccumulator(kMotionProfileSlot,
// mConstants.kMaxIntegralAccumulator,
// Constants.kLongCANTimeoutMs), mConstants.kName + ": Could not set max
// integral: ");

// TalonUtil.checkError(
// mMaster.config_IntegralZone(kMotionProfileSlot, mConstants.kIZone,
// Constants.kLongCANTimeoutMs),
// mConstants.kName + ": Could not set i zone: ");

// TalonUtil.checkError(
// mMaster.configAllowableClosedloopError(kMotionProfileSlot,
// mConstants.kDeadband,
// Constants.kLongCANTimeoutMs),
// mConstants.kName + ": Could not set deadband: ");

// TalonUtil.checkError(mMaster.config_kP(kPositionPIDSlot,
// mConstants.kPositionKp, Constants.kLongCANTimeoutMs),
// mConstants.kName + ": could not set kP: ");

// TalonUtil.checkError(mMaster.config_kI(kPositionPIDSlot,
// mConstants.kPositionKi, Constants.kLongCANTimeoutMs),
// mConstants.kName + ": could not set kI: ");

// TalonUtil.checkError(mMaster.config_kD(kPositionPIDSlot,
// mConstants.kPositionKd, Constants.kLongCANTimeoutMs),
// mConstants.kName + ": could not set kD: ");

// TalonUtil.checkError(mMaster.config_kF(kPositionPIDSlot,
// mConstants.kPositionKf, Constants.kLongCANTimeoutMs),
// mConstants.kName + ": Could not set kF: ");

// TalonUtil.checkError(
// mMaster.configMaxIntegralAccumulator(kPositionPIDSlot,
...
// TalonUtil.checkError(
// mMaster.configMotionCruiseVelocity(mConstants.kCruiseVelocity,
// Constants.kLongCANTimeoutMs),
// mConstants.kName + ": Could not set cruise velocity: ");

// TalonUtil.checkError(mMaster.configMotionAcceleration(mConstants.kAcceleration,
// Constants.kLongCANTimeoutMs),
// mConstants.kName + ": Could not set acceleration: ");

// TalonUtil.checkError(mMaster.configOpenloopRamp(mConstants.kRampRate,
// Constants.kLongCANTimeoutMs),
// mConstants.kName + ": Could not set voltage ramp rate: ");

// TalonUtil.checkError(mMaster.configClosedloopRamp(mConstants.kRampRate,
// Constants.kLongCANTimeoutMs),
...
// TalonUtil.checkError(
// mMaster.configVoltageCompSaturation(mConstants.kMaxVoltage,
// Constants.kLongCANTimeoutMs),
...
// mMaster.setStatusFramePeriod(StatusFrameEnhanced.Status_10_MotionMagic, 10,
// 20);
...
// mSlaves[i] =
// TalonFXFactory.createPermanentSlaveTalon(mConstants.kSlaveConstants[i].id,
...
// // The accel term can re-use the velocity unit conversion because both input
// and
...
// DriverStation.reportError(mConstants.kName + ": Talon Fault! " +
// mFaults.toString(), false);
// mMaster.clearStickyFaults(0);
// }
// if (mMaster.getControlMode() == ControlMode.MotionMagic) {
// mPeriodicIO.active_trajectory_position = (int)
// mMaster.getActiveTrajectoryPosition();

// if (mPeriodicIO.active_trajectory_position < mReverseSoftLimitTicks) {
// DriverStation.reportError(mConstants.kName + ": Active trajectory past
// reverse soft limit!", false);
// } else if (mPeriodicIO.active_trajectory_position > mForwardSoftLimitTicks) {
// DriverStation.reportError(mConstants.kName + ": Active trajectory past
// forward soft limit!", false);
// }
// final int newVel = (int) mMaster.getActiveTrajectoryVelocity();
// if (Util.epsilonEquals(newVel, mConstants.kCruiseVelocity, Math.max(1,
// mConstants.kDeadband)) || Util
// .epsilonEquals(newVel, mPeriodicIO.active_trajectory_velocity, Math.max(1,
// mConstants.kDeadband))) {
...
// .signum(newVel - mPeriodicIO.active_trajectory_velocity) *
...
// mPeriodicIO.velocity_ticks_per_100ms = (int)
...
// double abs_raw_no_rollover =
// mMaster.getSensorCollection().getIntegratedSensorAbsolutePosition();
// double abs_raw_with_rollover = abs_raw_no_rollover % 4096;
// return abs_raw_with_rollover + (abs_raw_with_rollover < 0 ?
// abs_raw_with_rollover + 4096 : 0);
// }
...
// } else if (mControlState == ControlState.POSITION_PID || mControlState ==
// ControlState.MOTION_PROFILING) {
...
// System.out.println(mConstants.kName + ": Master Talon reset occurred;
// resetting frame rates.");
// mMaster.setStatusFramePeriod(StatusFrameEnhanced.Status_2_Feedback0, 10, 20);
// mMaster.setStatusFramePeriod(StatusFrameEnhanced.Status_10_MotionMagic, 10,
// 20);
// mMaster.setStatusFramePeriod(StatusFrameEnhanced.Status_8_PulseWidth,
...
// public synchronized void setSetpointMotionMagic(double units, double
// feedforward_v) {
...
// public synchronized void setSetpointPositionPID(double units, double
// feedforward_v) {
// mPeriodicIO.demand = constrainTicks(homeAwareUnitsToTicks(units));
// double feedforward_ticks_per_100ms =
// unitsPerSecondToTicksPer100ms(feedforward_v);
// mPeriodicIO.feedforward = feedforward_ticks_per_100ms * (mConstants.kKf +
// mConstants.kKd / 100.0) / 1023.0;
...
// public synchronized void setGoalMotionProfiling(MotionProfileGoal goal,
...
// ticksToHomedUnits(mPeriodicIO.active_trajectory_position +
// lookahead_secs * mPeriodicIO.active_trajectory_velocity +
// 0.5 * mPeriodicIO.active_trajectory_acceleration * lookahead_secs *
// lookahead_secs);
...
// mPeriodicIO.position_units);
// SmartDashboard.putNumber(mConstants.kName + ": goal (units)",
// mPeriodicIO.demand);
// SmartDashboard.putBoolean(mConstants.kName + ": Homing Location",
// atHomingLocation());
// SmartDashboard.putNumber(mConstants.kName + " dt", mPeriodicIO.dt);
// SmartDashboard.putNumber(mConstants.kName + ": voltage",
// mPeriodicIO.output_voltage);
// }
...
// double estimated_pulse_pos = (mPeriodicIO.encoder_wraps * 4096) +
// mPeriodicIO.absolute_pulse_position_modded;
// double estimate_position_ticks =
// (mConstants.kMasterConstants.invert_sensor_phase ? -1 : 1)
