...
// final Translation2d latest_translational_displacement = new
// Translation2d(prev_swerve_pose_.getTranslation(),
// swerve_pose_.getTranslation());
// final Rotation2d latest_rotational_displacement =
// prev_swerve_pose_.getRotation().inverse().rotateBy(swerve_pose_.getRotation());

// Pose2d odometry_delta = new Pose2d(latest_translational_displacement,
// latest_rotational_displacement);

// final Pose2d measured_velocity = odometry_delta.scaled(1.0 / dt);
// final Pose2d measured_velocity_filtered =
// RobotState.getInstance().getSmoothedMeasuredVelocity();
// final Pose2d latest_velocity_acceleration =
// prev_swerve_velocity.inverse().transformBy(measured_velocity_filtered).scaled(1.0
// / dt);
// final Pose2d predicted_velocity =
// measured_velocity.transformBy(latest_velocity_acceleration.scaled(dt));

// mRobotState.addObservations(timestamp, odometry_delta, measured_velocity,
// predicted_velocity);
